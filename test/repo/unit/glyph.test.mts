...
test('lettering keeps its three rows and inverted source axis', () => {
  const part = REPO_GLYPH.parts[0]!
  expect((part.paths[0]!.match(/[Mm]/gu) ?? []).length).toBeGreaterThanOrEqual(
    3,
  )
  expect(part.transform).toMatch(/scale\([\d.]+ -[\d.]+\)/u)
  expect(part.transform.indexOf('translate')).toBeLessThan(
    part.transform.indexOf('scale'),
  )
})
