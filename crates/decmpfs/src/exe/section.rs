        ...
        if cmd == LC_SEGMENT_64 && name_eq(bytes.get(off + 8..off + 24)?, b"SMOL") {
            ...
            for _ in 0..nsects {
                ...
                if name_eq(bytes.get(soff..soff + 16)?, b"__DECMPFS") {
                    let size = u64::from_le_bytes(bytes.get(soff + 40..soff + 48)?.try_into().ok()?)
                        as usize;
                    let offset =
                        u32::from_le_bytes(bytes.get(soff + 48..soff + 52)?.try_into().ok()?)
                            as usize;
...
pub(crate) fn find_footer(bytes: &[u8]) -> Option<&[u8]> {
    ...
    let len_off = magic_off.checked_sub(8)?;
    let payload_len =
        u64::from_le_bytes(bytes.get(len_off..len_off + 8)?.try_into().ok()?) as usize;
    let hash_off = len_off.checked_sub(8)?;
