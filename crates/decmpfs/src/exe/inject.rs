...
fn read_layout(bytes: &[u8]) -> Result<Layout, String> {
    ...
    let linkedit_lc_off = linkedit_lc_off
        .ok_or_else(|| "no __LINKEDIT segment to anchor the new section".to_string())?;
...
mod tests {
    ...
    fn minimal_macho64_with_linkedit(
        ...
        put_u32(&mut m, linkedit_lc_off + 4, SEGMENT_COMMAND_64_SIZE as u32)
            .expect("linkedit cmdsize");
